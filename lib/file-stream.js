...
class FileStream extends stream.Readable {
  ...
  _notify () {
    ...
    const p = this._piece
    this._torrent.store.get(p, async (err, buffer) => {
      this._notifying = false
      if (this.destroyed) return
      if (err) return this._destroy(err)

      // Arc: Decrypt chunk.
      if (this._torrent._arc_cipherKey) {
        const AES_BLOCK_SIZE = 16
        const counter = (p * this._torrent.pieceLength) / AES_BLOCK_SIZE
        const iv = numberToArrayBuffer(counter)
        const algo = { name: 'AES-CTR', counter: iv, length: 128 }
        const key = this._torrent._arc_cipherKey
        const arrayBuffer = await crypto.subtle.decrypt(algo, key, buffer)
